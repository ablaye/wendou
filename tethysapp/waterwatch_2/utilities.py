...
try:
    credentials=ee.ServiceAccountCredentials('gtondapu@airquality-255511.iam.gserviceaccount.com', '/home/tethys/Downloads/waterwatch_2.json')
...
ponds = ee.FeatureCollection('projects/servir-wa/services/ephemeral_water_ferlo/ferlo_ponds').map(addArea).filter(ee.Filter.gt("area",10000))
...
def initLayers():
    return iobj['tile_fetcher'].url_format, regionImgID, communeImgID, arrondissementImgID, mndwiImg['tile_fetcher'].url_format, villageImgID
